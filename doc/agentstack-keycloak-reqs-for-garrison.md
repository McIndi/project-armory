...
| `auth.nextauthSecret` | If empty, the UI secret template auto-generates `randAlphaNum 32` and **persists it** via a `lookup` of `agentstack-ui-secret` (key `authSecret`). | Simplest: leave empty and let the chart own/persist it. If you want it under OpenBao control, supply it as a Helm value too (no `existingSecret` path exists). |
...
- **Target namespace (DECISION — must be made, then enumerated)**: The doc never
  ...
  VSO `VaultStaticSecret` destination namespace for client secrets (§3.3, §6), the
  trust-manager `Bundle` target namespaces for CA distribution (§4.3), and the
  hostAlias patch (§4.1) all need it. Pick a convention (e.g. `agentstack`) up
  front and apply it consistently across the Helm release namespace, the VSO
  resources, and the trust-manager Bundle targets. This is a decision, not a
  discoverable fact — but it must be pinned before building.
...
- [ ] Run the §9 verification matrix after first deploy; each row is a concrete
      pass/fail gate (token `aud`, role claim, UI login, authenticated API call).
- [ ] Follow the §8 first-deploy ordering; out-of-order apply is a common
      first-run failure (e.g. Helm release referencing an `existingSecret` that
      VSO has not yet synced).

## 8. First-deploy ordering (dependency sequence)

First-run reliability depends on applying these in order — most early failures
are an ordering problem, not a config problem. Each step's outputs are the next
step's inputs.

1. **Namespace** — create garrison's AgentStack namespace (§5). Everything below
   targets it.
2. **CA trust** — distribute the internal (`pki-int`) and external (`pki-ext`) CA
   bundles into that namespace via trust-manager `Bundle` (§4.3). Needed before
   any pod or provisioning task does TLS validation.
3. **Realm + clients + roles + audience scope** — provision the `agentstack` realm
   and its objects in armory's Keycloak via the admin REST API (§3, §6). This is
   where the client secrets are generated.
4. **Client-secret Secret** — store the generated client secrets in OpenBao KV and
   let VSO sync them into the k8s Secret named by
   `externalOidcProvider.existingSecret` (§3.3). **This Secret must exist before
   step 6** — the Helm release references it and pods will crash-loop otherwise.
5. **`encryptionKey` / `nextauthSecret`** — resolve the non-`existingSecret`
   values (§4.5): generate `encryptionKey`, stage it (and optionally
   `nextauthSecret`) for injection as Helm values at the next step.
6. **Helm release** — deploy the chart (`keycloak.enabled: false`,
   `externalOidcProvider.*` populated, `trustProxyHeaders: true`, `encryptionKey`
   passed as a value). The hostAlias patch (§4.1) and UI ingress (§4.6) land here.
7. **Verify** — run §9.

## 9. Verification / acceptance criteria

Concrete pass/fail checks for the open build items. "Done" = every row passes.

| # | What to verify | Concrete check | Pass criterion |
|---|---|---|---|
| 1 | Audience mapping (§3.4) | Decode the UI-minted **access** token (`jwt` decode of a captured token, or Keycloak token endpoint) | `aud` array contains the literal `agentstack-server` (not a URL) |
| 2 | Role claim (§3.5) | Decode the same token | `realm_access.roles` contains `agentstack-admin` for the seed admin |
| 3 | Server audience validation (§2) | Authenticated API call to the server with a valid UI token | Returns 2xx, **not 401** (`AUTH__OIDC__VALIDATE_AUDIENCE=true` path) |
| 4 | UI login (§4.1) | Browse to the UI host, complete the Keycloak login redirect | Lands back on the UI authenticated; no issuer/redirect-URI mismatch error |
| 5 | In-pod issuer resolution (§4.1) | `nslookup`/curl the public issuer from inside a UI/server pod | Resolves via the hostAlias to the ingress IP; OIDC discovery succeeds |
| 6 | TLS trust (§4.3) | OIDC discovery + token validation from the pod over the public HTTPS issuer | No certificate-verification errors (external CA trusted) |
| 7 | `encryptionKey` present (§4.5) | Inspect the rendered `agentstack-secret` | `encryptionKey` is non-empty (empty silently breaks persistence) |
| 8 | Proxy headers (§4.4) | Confirm `TRUST_PROXY_HEADERS` on UI + server pods | Set to `true` via `trustProxyHeaders`, no Deployment patches present |

## 10. Secret rotation & rollback

Rotation characteristics differ sharply per secret — do **not** assume one
procedure fits all.

- **OIDC client secrets (`agentstack-ui` / `agentstack-server`) — safe, knowable.**
  Rotate via the Keycloak client-secret endpoint (armory's
  [`headlamp/tasks/oidc_client.yml`](../ansible/roles/headlamp/tasks/oidc_client.yml)
  has a working "rotate when explicitly provided" task to adapt), update the value
  in OpenBao KV, and let VSO resync the k8s Secret. Brief auth blips during the
  swap; no data loss.
- **`auth.nextauthSecret` — knowable consequence.** It signs UI session cookies;
  rotating it **invalidates all active UI sessions** (users must re-login). Safe,
  but schedule it. If chart-generated, it persists via the `agentstack-ui-secret`
  lookup, so it only rotates when you deliberately change it.
- **`encryptionKey` — potentially DESTRUCTIVE, NOT knowable statically.** It feeds
  `PERSISTENCE__ENCRYPTION_KEY` and encrypts data **at rest in the database**.
  Rotating it likely renders previously-encrypted rows **undecryptable** unless
  beeai ships a key-rotation / re-encryption path — which is unconfirmed. Treat
  rotation as a data-migration event: confirm app support first, back up the DB,
  and have a rollback (restore old key + DB snapshot). Do **not** rotate it
  casually as part of routine secret hygiene.
