...
CA_BUNDLE = Path(__file__).resolve().parents[1] / "vault" / "ca-bundle.pem"
...
def test_tls_handshake_version(vault_env):
    cafile = CA_BUNDLE if CA_BUNDLE.exists() else (TLS_DIR / "ca.crt")
...
def test_live_cert_issuer_after_cutover(vault_env):
    internal_cert_path = TLS_DIR / "vault-internal.crt"
    if not internal_cert_path.exists():
        return

    cert = _load_cert(internal_cert_path)
