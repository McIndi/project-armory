...
| `nginx_ingress_ip` | `192.168.0.150` | External IP assigned to ingress service. |
...
| `nginx_ingress_firewall_manage` | `true` | Whether to open NodePort firewall ports. |
| `nginx_ingress_firewall_zone` | `public` | firewalld zone for NodePort rules. |

## Task flow
1. Install cert-manager via Helm and wait for webhook readiness (`install.yml`).
2. Install ingress-nginx via Helm with NodePort 30080/30443 (`install.yml`).
3. Open firewall rules for ingress NodePorts when enabled (`install.yml`).
...
- ingress reachable on NodePort but not domain.
  Action: verify DNS/hosts mapping for `nginx_ingress_domain` and firewall rules.
